...
int main(int, char **) {
  ...
  // Create window
  GLFWwindow *window =
      glfwCreateWindow(1600, 900, "LIDAR Point Cloud Viewer", NULL, NULL);
  if (window == NULL)
    return 1;
  ...
  const char *colorModeNames[] = {"RGB Colors", "Height Map", "Intensity",
                                  "Uniform White"};
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    if (!io.WantCaptureMouse && g_ScrollOffset != 0) {
      renderer.getCamera().zoom(g_ScrollOffset);
      g_ScrollOffset = 0;
    }
    ...
    if (showControlPanel) {
      ...
      if (showGrid) {
        ...
        if (ImGui::SliderFloat("Grid Spacing", &gridSpacing, 0.1f, 10.0f,
                               "%.1f units")) {
      ...
      if (ImGui::Button("Top",
                        ImVec2(ImGui::GetContentRegionAvail().x * 0.48f, 0))) {
    ...
    // Statistics overlay
    if (showStats) {
      ImGui::SetNextWindowPos(ImVec2(io.DisplaySize.x - 250, 10),
                              ImGuiCond_Always);
