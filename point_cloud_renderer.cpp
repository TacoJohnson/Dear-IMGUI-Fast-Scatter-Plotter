...
void Camera::getProjectionMatrices(float modelview[16], float projection[16],
                                   int viewport[4], int width, int height) {
...
PointCloudRenderer::PointCloudRenderer()
    : pointCount(0), pointSize(2.0f), colorMode(COLOR_RGB), showGrid(true),
      gridSpacing(1.0f), gridSize(10), showAxisLabels(true) {
  minX = minY = minZ = 0;
  maxX = maxY = maxZ = 0;
  setupOpenGL();
}

PointCloudRenderer::~PointCloudRenderer() { cleanupOpenGL(); }
...
void PointCloudRenderer::renderGrid() {
  ...
  // Draw 3D tick marks at grid intervals
  glBegin(GL_LINES);
  float tickSize = gridSpacing * 0.2f;
  ...
  for (int i = 0; i <= numLinesX; ++i) {
    ...
    if (x > maxX)
      x = maxX;
    glVertex3f(x, minY, minZ);
    glVertex3f(x, minY - tickSize, minZ);
  }
  ...
  for (int i = 0; i <= numLinesY; ++i) {
    ...
    if (y > maxY)
      y = maxY;
    glVertex3f(minX, y, minZ);
    glVertex3f(minX - tickSize, y, minZ);
  }
  ...
  for (int i = 0; i <= numLinesZ; ++i) {
    ...
    if (z > maxZ)
      z = maxZ;
    glVertex3f(minX, minY, z);
    glVertex3f(minX - tickSize, minY, z);
  }
...
static bool project3Dto2D(float objX, float objY, float objZ,
                          ...
                          const int viewport[4], float &winX, float &winY) {
  ...
  GLdouble wx, wy, wz;
  GLint result = gluProject((GLdouble)objX, (GLdouble)objY, (GLdouble)objZ, mv,
                            proj, viewport, &wx, &wy, &wz);

  if (result == GL_TRUE && wz >= 0.0 && wz <= 1.0) {
    winX = (float)wx;
    winY = (float)viewport[3] - (float)wy; // Flip Y for screen coordinates
    return true;
  }
  return false;
}
...
void PointCloudRenderer::renderAxisLabels(int screenWidth, int screenHeight) {
  ...
  int viewport[4];
  camera.getProjectionMatrices(modelview, projection, viewport, screenWidth,
                               screenHeight);
  ...
  // Pre-convert matrices for efficiency
  GLdouble mv[16], proj[16];
  for (int i = 0; i < 16; i++) {
    mv[i] = (GLdouble)modelview[i];
    proj[i] = (GLdouble)projection[i];
  ...
  auto drawAt = [&](float wx, float wy, float wz, const char *text, ImU32 color,
    ...
    GLdouble sx, sy, sz;
    if (gluProject((GLdouble)wx, (GLdouble)wy, (GLdouble)wz, mv, proj, viewport,
                   &sx, &sy, &sz) == GL_TRUE &&
        sz >= 0.0 && sz <= 1.0) {

      float fx = (float)sx;
      float fy = (float)viewport[3] - (float)sy;

      ImVec2 tsize = ImGui::CalcTextSize(text);
      ImVec2 tpos(fx - tsize.x / 2 + offset.x, fy - tsize.y / 2 + offset.y);
  ...
  for (int i = 0; i <= numLinesX; i += skipX) {
    ...
    drawAt(x, minY, minZ, label, IM_COL32(255, 150, 150, 255), ImVec2(0, 15));
  }
  drawAt(maxX + gridSpacing * 0.5f, minY, minZ, "X", IM_COL32(255, 0, 0, 255));
  ...
  for (int i = 0; i <= numLinesY; i += skipY) {
    ...
    drawAt(minX, y, minZ, label, IM_COL32(150, 255, 150, 255), ImVec2(-25, 0));
  }
  drawAt(minX, maxY + gridSpacing * 0.5f, minZ, "Y", IM_COL32(0, 255, 0, 255));
  ...
  for (int i = 0; i <= numLinesZ; i += skipZ) {
    ...
    drawAt(minX, minY, z, label, IM_COL32(150, 150, 255, 255), ImVec2(-25, 15));
  }
  drawAt(minX, minY, maxZ + gridSpacing * 0.5f, "Z", IM_COL32(0, 0, 255, 255));
}
